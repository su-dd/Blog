...
tags:
  - DDD
---
上篇介绍了值对象、DP，这里希望在DP的基础上，说清楚领域驱动设计的其他主要概念。


## 业务要求
> 1、业务方法的入参是客户姓名和手机号，首先使用手机号去调用外部服务查询实名信息，校对是否和入参中的姓名一致，如果一致，则通过。
> 
> 2、然后，然后根据得到的实名信息，按照一定逻辑计算得出该用户的标签，该标签将作为用户的一个属性。
> 
...
```
这里的RealnameInfo，就是**DP**；同理，**Name**，**PhoneNumber**也是**DP**。
...
```
这里的RegistrationService, 就是**应用服务**。
